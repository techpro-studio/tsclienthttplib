...
export class DefaultRequestPerformer implements RequestPerformer {
  constructor(@inject(injectTypes.baseURLHolder) private baseURLHolder: BaseURLHolder) {}
  ...
  public async performRequest<T>(request: RequestParams): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      xhr.open(request.method.toString(), withQuery(this.baseURLHolder.baseURL + request.relativeURL, request.query));
      ...
      xhr.onload = () => {
        try {
          ...
          if (xhr.status < 400) {
            ...
            const serverError: ServerError = Object.assign(serverErrorBody, { statusCode: xhr.status });
            reject(serverError);
          }
        } catch (e) {
          reject({
